...
class DynamicIconPanel extends JPanel {
  ...
  public void addCircle() {
    ...
    try {
      ...
      // Update animation state or group and it's decendants so that it reflects
      // new animation values.
      // We could also call diagram.update(0.0) or
      // SVGCache.getSVGUniverse().update(). Note that calling
      // circle.update(0.0) won't display anything since even though it will
...
public class TestPanel extends javax.swing.JFrame {
  ...
  private void bn_frontActionPerformed(java.awt.event.ActionEvent evt) {
    panel.setCircleForeground(new Color((float) Math.random(), (float) Math
        .random(), (float) Math.random()));
    repaint();
  }

  private void bn_backActionPerformed(java.awt.event.ActionEvent evt) {
    panel.setCircleBackground(new Color((float) Math.random(), (float) Math
        .random(), (float) Math.random()));
    repaint();
  }
